...
const COURSE_TYPES = [
  "Aperitif", "Appetizer", "Warm Appetizer", "Soup", 
  "Main Course", "Side Dish", "Salad", "Dessert", 
  "Cheese", "Digestif", "Drink", "Snack"
];
          ...
          {eventData.courses.map((course, index) => (
            <div key={index} className="course-card-item">
               <div className="course-move-controls">
                  <button className="btn-move" onClick={() => moveCourse(index, 'up')} disabled={index === 0}>▲</button>
                  <button className="btn-move" onClick={() => moveCourse(index, 'down')} disabled={index === eventData.courses.length - 1}>▼</button>
               </div>
               <img src={course.recipe.image || "https://via.placeholder.com/60"} alt="" className="course-card-img"/>
               <div className="course-card-info">
                  <select className="course-type-select" value={course.courseType} onChange={(e) => handleCourseTypeChange(index, e.target.value)}>
                     {COURSE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <h4 className="course-recipe-title">{course.recipe.title}</h4>
               </div>
               <button className="btn-remove-course" onClick={() => removeCourse(index)}>✕</button>
            </div>
          ))}
        </div>
      ) : (
        <div style={{padding:'2rem', border:'2px dashed #ccc', borderRadius:'12px', textAlign:'center', marginBottom:'1rem', opacity: 0.6}}>
           🍽️ Tvoj jelovnik je prazan.
        </div>
      )}
        ...
        <div className="modal-backdrop" onClick={() => setIsRecipeModalOpen(false)}>
           <div className="modal-content" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                 <h3>Dodaj u Jelovnik</h3>
                 <button className="btn-close" onClick={() => setIsRecipeModalOpen(false)}>&times;</button>
              </div>
              <div className="form-group">
                 <label>Vrsta Slijeda</label>
                 <select className="form-select" value={currentCourseType} onChange={(e) => setCurrentCourseType(e.target.value)}>
                   {COURSE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                 </select>
              </div>
              <div className="form-group">
                <input type="text" className="modal-search-input" placeholder="Traži recept..." value={recipeSearchQuery} onChange={(e) => setRecipeSearchQuery(e.target.value)} autoFocus />
              </div>
              <div className="recipe-selection-list" style={{maxHeight: '300px', overflowY: 'auto'}}>
                 {filteredRecipes.map(recipe => (
                    <div key={recipe._id} className="recipe-select-item" onClick={() => addCourse(recipe)}>
